...
export function VoiceSection({ form, onFieldChange }: VoiceSectionProps) {
  const { t } = useTranslation()

  return (
    <ConfigSectionCard title={t("pages.config.sections.voice")}>
      <Field
        label={t("pages.config.voice_language")}
        hint={t("pages.config.voice_language_hint")}
        layout="setting-row"
      >
        <Input
          value={form.voiceLanguage}
          onChange={(e) => onFieldChange("voiceLanguage", e.target.value)}
          placeholder={t("pages.config.voice_language_placeholder")}
        />
      </Field>
